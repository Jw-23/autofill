---
name: Local_AI_MODEL_API
description: gooogle chrome 本地AI NANO的 API使用指南，Prompt API, 创建模型，格式化输出
---
...
All of the built-in AI APIs are available on `localhost` in Chrome. Set the
following flags to **Enabled**:

- `chrome://flags/#optimization-guide-on-device-model`
- `chrome://flags/#prompt-api-for-gemini-nano-multimodal-input`

Then click **Relaunch** or restart Chrome. If you encounter errors,
[troubleshoot localhost](https://developer.chrome.com/docs/ai/get-started#troubleshoot_localhost).
...
let params=await LanguageModel.params();
const slightlyHighTemperatureSession = await LanguageModel.create({
  temperature: Math.max(params.defaultTemperature * 1.2, 2.0),
  topK: params.defaultTopK,
});
